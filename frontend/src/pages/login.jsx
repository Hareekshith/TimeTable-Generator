...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isNew) {
        // Register new user
        await axios.post('http://localhost:5000/register', { username: un, password: psswd });
        alert("Registration successful! Please log in.");
        setIsNew(false);
      } else {
        // Login existing user
        const response = await axios.post('http://localhost:5000/login', { username: un, password: psswd });
